...
> This package is fantastic, but I wanted to add some customisation. Hence this fork in my repo.


## Installation

You can install the original `ggforestplot` from NightingaleHealth, or this fork to get
my quality of life additions (described below):
...
If you want display package vignettes with `utils::vignette()`, install with `build_vignettes = TRUE`. 

## What's new in v0.1.1 (this fork)?

This fork adds several quality-of-life features to `ggforestplot::forestplot()`:

 - `est_table`: print an aligned estimate + CI column to the right of the plot.
 - `name = c(col1, col2, ...)`: use multiple columns for y-axis labels (to create table-like y-axis labels).
 - `alpha`: make non-significant results semi-transparent.
 - `filled_nonsig`: optionally keep non-significant points filled (useful with `alpha`).
...
When grouping with `colour` or `shape`, the estimate table is dodged to match
the plotted rows.
...
Helps to draw reader's eye to significant results, rather than the
(much wider CIs) non-significant results. Note I also set `filled_nonsig = TRUE`
as the default when providing a `pvalue` is for the shape to be hollow.
